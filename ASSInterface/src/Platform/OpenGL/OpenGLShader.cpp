...
namespace ASSInterface {
	...
	static GLenum ShaderTypeFromString(const std::string& type)
	{
		...
		ASS_CORE_ASSERT(false, "Unknown shader type!");
		return 0;
	}
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source)
	{
		...
		while (pos != std::string::npos)
		{
			...
			ASS_CORE_ASSERT(eol != std::string::npos, "Syntax error");
			size_t begin = pos + typeTokenLength + 1;
			std::string type = source.substr(begin, eol - begin);
			ASS_CORE_ASSERT(ShaderTypeFromString(type), "Invalid shader type specified");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		ASS_CORE_ASSERT(shaderSources.size() <= 2, "We only support 2 shaders for now");
	...
	void OpenGLShader::UploadUniform1ui(const std::string& name, unsigned short value)
	{
		ASS_PROFILE_FUNCTION();
